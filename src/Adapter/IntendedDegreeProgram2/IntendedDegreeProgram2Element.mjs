...
export class IntendedDegreeProgram2Element extends HTMLElement {
     ...
     */
    static new(css_api, intended_degree_program_2, chosen_intended_degree_program_2_function, back_function = null) {
        return new this(
            css_api,
            intended_degree_program_2,
            chosen_intended_degree_program_2_function,
     ...
     * @private
     */
    constructor(css_api, intended_degree_program_2, chosen_intended_degree_program_2_function, back_function) {
        super();
    ...
    #render() {
        ...
        subject_element.innerText = this.#intended_degree_program_2.subject.label;

        const combination = this.#intended_degree_program_2.subject.combinations.find(_combination => _combination.id === this.#intended_degree_program_2.combination);

        const combination_element = this.#form_element.addInput(
            "Combination of Subjects",
            "readonly"
        );
        combination_element.innerText = combination.label;

        const mandatory_element = this.#form_element.addInput(
            "Mandatory Subjects",
            "readonly"
        );
        mandatory_element.innerText = combination.mandatory ?? "";
